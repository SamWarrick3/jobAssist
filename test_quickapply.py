...
class TestQuickApply():
    ...
    def find_element(self, start, type, tag: TAGS, contains: list, failure_level: FAILURE_LEVELS, case_insensitive=True):
        ...
        Returns:
            _type_: _description_
        """
        # master method to find an element
        # parameters: 
                # starting point (could be an element could be the whole xml)
                # element type (could also be no type and looking based on text in element)
                # contains list (will contain the search criteria)
        elements_found = []
        idx = 0
    ...
    def fill_first_name(self):
        """_summary_
        """
        first_name_label = self.find_label(FIRST_NAME_FORMATS, FAILURE_LEVELS.fail)
        # check there is an input element under the label
    ...
    def fill_last_name(self):
        """_summary_
        """
        last_name_label = self.find_label(LAST_NAME_FORMATS, FAILURE_LEVELS.fail)
        # check there is an input element under the label
    ...
    def fill_email(self):
        """_summary_
        """
        email_label = self.find_label(EMAIL_FORMATS, FAILURE_LEVELS.boolean)
        if email_label != False:
            if self.find_input_element(email_label) != None:
                self.find_input_element(email_label)[0].send_keys("swarrick6@gmail.com")
            else:
                LOG.info("Email label found but no email input box found")
        else:
    ...
    def fill_phone(self):
        """_summary_
        """
        phone_label = self.find_label(BASIC_PHONE_FORMATS, FAILURE_LEVELS.boolean)
    ...
    def fill_zipcode(self):
        """_summary_
        """
        zip_label = self.find_label(ZIPCODE_FORMATS, FAILURE_LEVELS.boolean)
        if zip_label != False:
    ...
    def fill_country(self):
        """_summary_
        """
        country_label = self.find_label(COUNTRY_FORMATS, FAILURE_LEVELS.boolean)
