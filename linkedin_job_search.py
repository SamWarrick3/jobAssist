...
LOCATION_SEARCH_CRITERIA = "Canada"
...
POSSIBLE_DATE_RANGES = {"any time": "Filter by Any time",
                     "month": "Filter by Past month",
                     "week": "Filter by Past week",
                     "24hr": "Filter by Past 24 hours"}
DATE_RANGE = POSSIBLE_DATE_RANGES["24hr"]
...
def linkedin_job_search(qa: test_quickapply.TestQuickApply):
  ...
  qa.driver.find_element(By.XPATH, JOBDESCRIPTION_SEARCHBOX_XPATH).send_keys(JOB_SEARCH_CRITERIA)
  ...
  qa.wait.until(lambda d: qa.driver.find_element(By.XPATH, LOCATION_SEARCHBOX_XPATH).is_displayed())
  qa.driver.find_element(By.XPATH, LOCATION_SEARCHBOX_XPATH).send_keys(LOCATION_SEARCH_CRITERIA)
  ...
  for element in menu_items:
    elements = qa.driver.find_elements(By.XPATH, f".//span[text()='{DATE_RANGE}']")
    if len(elements) > 0:
    ...
    if len(no_jobs_element) == 0:
      ...
      for job in jobs_list: # iterate through all jobs on page
        ...
        try:
          ...
          test_quickapply.LOG.error(f"Caught the following error. Will attempt to continue execution.\n{e}")
  ...
  csv_name = f"todays_jobs_{LOCATION_SEARCH_CRITERIA.replace(" ", "")}.csv"
